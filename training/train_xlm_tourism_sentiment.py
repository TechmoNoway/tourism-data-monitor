"""
Train XLM-RoBERTa Sentiment Classifier for Multilingual Comments
3-class classification: positive, negative, neutral

Usage:
    python training/train_xlm_sentiment.py --epochs 5 --batch_size 16 --device cuda
"""
...
def load_data_from_db() -> pd.DataFrame:
    """Load multilingual (non-Vietnamese) comments with sentiment labels from database"""
    print("Loading multilingual comments from database...")
    ...
    try:
        ...
        print(f"\nLoaded {len(df)} multilingual comments")
...
def main(args):
    ...
    print("="*80)
    print("XLM-RoBERTa Sentiment Classifier Training")
    print("="*80)
    ...
    model = XLMRobertaSentimentClassifier(n_classes=len(SENTIMENTS), dropout=args.dropout)
    ...
    for epoch in range(args.epochs):
        ...
        if val_f1_macro > best_f1_macro:
            ...
            }, 'training/models/xlm_sentiment_best_model.pt')
    ...
    checkpoint = torch.load('training/models/xlm_sentiment_best_model.pt')
    ...
    stats_df.to_csv('training/models/xlm_sentiment_statistics.csv', index=False)
    print("\n✅ Saved statistics to training/models/xlm_sentiment_statistics.csv")
    
    # Language-specific statistics
    lang_stats = df.groupby(['language', 'sentiment']).size().unstack(fill_value=0)
    lang_stats.to_csv('training/models/xlm_sentiment_language_stats.csv')
    print("✅ Saved language statistics to training/models/xlm_sentiment_language_stats.csv")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train XLM-RoBERTa Sentiment Classifier')
